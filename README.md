...
#### Предложение
Деплоить индекс из конструктора NftRoot. Это позволит создавать NftRoot одной транзакцией пользователя. Это решение не очень важно, когда NftRoot контракт один, и его деплоит администратор, но становится актуально, когда пользователь может задеплоить множество рутов, каждый из которых создаётся под отдельную коллекцию
...
Ниже показана схема минтинга токенов с использованием контракта Storage. Создание токена происходит несколько этапов:
...
5. Деплоятся индексы, и минтинг считается завершённым
...
#### Предложения
Если защита нужна, то вместо `addrTo != address(0)` лучше использовать `addrTo.value != 0`, так как это защитит от оправки на нулевой адрес `-1` воркчейна
...
* Добавить в метод `getInfo()` входящий параметр `TvmCell payload`, который он же будет возвращать в выходных параметрах. Это нужно для того, чтобы другие контракты могли пользоваться этим методом когда нужно пробрасывать какие-то дополнительные данные
...
### 7. Добавление в метод `transferOwnership()` контракта Data параметров `address notificationReceiver`, `TvmCell payload`
...
#### Предложение
Добавить параметр `uint128 value` в метод `destruct()` контракта Index. Значение `value`, передаваемое в метод `destruct()`, не должно быть захардкожено, а должно задаваться изменяемым параметром или передаваться извне

### 10. Возвращение остатков от транзакции
#### Предложение
Не хранить остатки от транзакций в Data и NftRoot, а возращать остатки на кошелёк, который может указать пользователь. Это позволит не блокировать деньги на контрактах, а также отказаться от методов, вроде, `withdraw()`. Примеры методов, которые не возвращают деньги:
...
#### Предложение
Использовать подход itgod

### 13. Поиск токенов, выставленных на продажу
При разработке приложений, использующих NFT, практически всегда встаёт задача поиска токенов, выставленных на продажу. Эта задача решается двумя способами:
...
#### Предложение
Использовать второй подход с одним `owner`, так как это не имеет никаких недостатков по сравнению с первым подходом, но при этом упрощает логику управления
...
[Цитата](https://github.com/itgoldio/everscale-tnft#data): "Если нам нужно изменить Index контракт - нам нужно заново деплоить Data контракты т.к. в них не заложены возможности установки нового кода Index и передеплоивания его."
...
Текущая реализация решения этой проблемы не работает.  Если обновить код Index, и вызвать после этот метод `redeployIndex()`, то он не удалит старые индексы. Причина: код Index контракта будет уже новый, у не получится зарезолвить адреса старых Index контрактов, чтобы вызвать у них метод `destruct()`.
...
* Может ли быть токен, у которого нет Storage?
* Какие индексы использовать для быстрого поиска контрактов? On-chain при помощи `Index` контрактов, как это реализовано в TNFT, или off-chain, как это реализовано в DApp?



## Визуализация взаимодействия между контрактами. Исходник с легендой схемы
